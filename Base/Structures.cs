...
namespace Edge.Structures
{
    ...
    public class Trie<T,V> : ITrieNode<T,V>, IDictionary<IEnumerable<T>,V>
    {
        ...
        private readonly IEqualityComparer<IEnumerable<T>> _comp;
        public Trie(IEqualityComparer<IEnumerable<T>> comp = null)
        {
        ...
        {
            bool included;
            var match = _children.First(a => key.StartsWith(a.Key), out included);
            if (!included)
            {
                value = default(V);
                return false;
        ...
        private void Add(IList<T> key, V value, IEnumerable<T> fullkey)
        {
            ...
            bool included;
            var match = _children.First(a => a.Key.StartsWith(key), out included);
            if (included)
            {
                if (match.Key.CompareCount(key) != 0)
                {
                    ...
                    var newchild = new Trie<T,V>(_comp);
                    newchild.Add(new T[0], value, fullkey);
                    newchild._children[match.Key.Splice(key.Count)] = match.Value;
            ...
            match = _children.First(a => a.Key.Any() && key.StartsWith(a.Key), out included);
            if (included)
            {
                ...
                if (trie == null)
                {
                    ...
                    trie = new Trie<T,V>(_comp);
                    trie.Add(new T[0], match.Value.First().Value, match.Value.First().Key);
                    _children[match.Key] = trie;
                }
                trie.Add(key.Splice(match.Key.Count), value, fullkey);
                return;
            }
            var submatch = _children.Attach(a => a.Key.SharedPrefix(key)).First(a => a.Item2.Any(), out included);
            if (included)
            {
                ...
                _children.Remove(match.Key);
                var newchild = new Trie<T,V>(_comp);
                newchild.Add(key.Splice(prefix.Length), value, fullkey);
                newchild._children[match.Key.Splice(prefix.Length)] = match.Value;
        ...
        public IEnumerable<KeyValuePair<IEnumerable<T>, V>> PrefixQuery(IEnumerable<T> prefix)
        {
            if (!prefix.Any())
                return this;
            bool any;
            var match = this._children.First(a => a.Key.SharedPrefix(prefix).Any(), out any);
            if (!any)
    ...
    public class Trie<T> : ITrieNode<T>, ICollection<IEnumerable<T>>
    {
        ...
        private void Add(IList<T> key, IEnumerable<T> fullkey)
        {
            ...
            if (included)
            {
                ...
                {
                    _children.Remove(match);
                    trie = new Trie<T>(_comp);
                    trie.Add(new T[0], match.Value.First());
                    _children[match.Key] = trie;
                }
                trie.Add(key.Splice(match.Key.Count), fullkey);
                return;
            }
            var submatch = _children.Attach(a => a.Key.SharedPrefix(key)).First(a => a.Item2.Any(), out included);
            if (included)
            {
                ...
                _children.Remove(match.Key);
                var newchild = new Trie<T>(_comp);
                newchild.Add(key.Splice(prefix.Length), fullkey);
                newchild._children[match.Key.Splice(prefix.Length)] = match.Value;
                _children[prefix] = newchild;
                return;
            }
