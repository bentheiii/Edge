...
namespace Edge.Funnels
{
    ...
    public class PrefixFunnel<RT> : QualifierFunnel<string,string,RT>
    {
        public PrefixFunnel(bool removePrefix = true) : base((s, s1) => s.StartsWith(s1))
        {
            RemovePrefix = removePrefix;
        }
        public bool RemovePrefix { get; }
        public void Add(Proccesor<string, RT> p)
        {
            Add("", p);
        }
        public override void Add(string prefix,Proccesor<string, RT> p) 
        {
            // ReSharper disable once ImplicitlyCapturedClosure
            base.Add(prefix,(string processed, out RT returnval) => p(RemovePrefix? processed.Substring(prefix.Length) : processed, out returnval));
        }
    }
    public class PrefixFunnel : QualifierFunnel<string, string>
    {
        public PrefixFunnel(bool removePrefix = true) : base((s, s1) => s.StartsWith(s1))
        {
            RemovePrefix = removePrefix;
        ...
        public bool RemovePrefix { get; }
        public void Add(Proccesor<string> p)
        {
            Add("", p);
        }
        public override void Add(string prefix, Proccesor<string> p)
        {
            // ReSharper disable once ImplicitlyCapturedClosure
            base.Add(prefix,processed => p(RemovePrefix ? processed.Substring(prefix.Length) : processed));
        }
    }
}
